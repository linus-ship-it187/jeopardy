...
function watchBroadcasters(){
  db.ref('jeopardyRTC_vf182/peers').on('child_added', snap => {
  ...
  db.ref('jeopardyRTC_vf182/peers').on('child_removed', snap => {
    const peerId = snap.key;
...
function connectAsViewer(broadcasterPeerId, playerId){
  ...
  pc.onicecandidate = (event) => {
    if (event.candidate) {
      ...
      db.ref(`jeopardyRTC_vf182/signals/${broadcasterPeerId}/iceFromViewer/${myPeerId}`).push(event.candidate.toJSON());
    }
  };
  ...
  pc.createOffer()
    ...
    .then(() => {
      ...
      db.ref(`jeopardyRTC_vf182/signals/${broadcasterPeerId}/offersFromViewers/${myPeerId}`)
        .set(pc.localDescription.toJSON());
    })
    .catch(err => console.error('[WebRTC][Viewer] Fehler beim Erstellen des Offers:', err));

  db.ref(`jeopardyRTC_vf182/signals/${broadcasterPeerId}/answersToViewers/${myPeerId}`).on('value', snap => {
  ...
  db.ref(`jeopardyRTC_vf182/signals/${broadcasterPeerId}/iceFromBroadcaster/${myPeerId}`).on('child_added', snap => {
...
function startBroadcast(playerId, stream){
  ...
  db.ref('jeopardyRTC_vf182/peers/' + myPeerId).set({ playerId: playerId, ts: Date.now() });
  db.ref('jeopardyRTC_vf182/peers/' + myPeerId).onDisconnect().remove();
  db.ref('jeopardyRTC_vf182/signals/' + myPeerId).onDisconnect().remove();

  db.ref(`jeopardyRTC_vf182/signals/${myPeerId}/offersFromViewers`).on('child_added', snap => {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        db.ref(`jeopardyRTC_vf182/signals/${myPeerId}/iceFromBroadcaster/${viewerPeerId}`).push(event.candidate.toJSON());
      }
    };
    ...
    pc.setRemoteDescription(new RTCSessionDescription(offer))
      ...
      .then(() => {
        ...
        db.ref(`jeopardyRTC_vf182/signals/${myPeerId}/answersToViewers/${viewerPeerId}`).set(pc.localDescription.toJSON());
      })
      .catch(err => console.error('[WebRTC][Broadcaster] Fehler bei Antwort:', err));

    db.ref(`jeopardyRTC_vf182/signals/${myPeerId}/iceFromViewer/${viewerPeerId}`).on('child_added', s => {
...
function stopBroadcast(){
  db.ref('jeopardyRTC_vf182/peers/' + myPeerId).remove();
  db.ref('jeopardyRTC_vf182/signals/' + myPeerId).remove();
